...
Proposto por \citeonline{intsintorig} como um sistema de transcrição para
entoação capaz de ser utilizado para analisar múltiplas línguas, com objetivo de criar uma representação equivalente ao IPA (\emph{International Phonetic Alphabet}) para prosódia. 
% Utilizado em sistemas de anotação automática.
...
à marcação anterior, Higher (H), Same (S) e Lower (L), e dois símbolos ditos
iterativos, Upstepped (U) e Downstepped (D).

Além das anotações, utilizam-se dois parâmetros fixos \emph{key}, definido em
Hertz, e \emph{span}, definido em oitavas.
...
Utilizada por \citeonline{intsintpt} e \citeonline{moraes1998} para analisar entoação para o português brasileiro.
...
e cita os elementos \emph{emphasis}, \emph{break} e \emph{prosody}
